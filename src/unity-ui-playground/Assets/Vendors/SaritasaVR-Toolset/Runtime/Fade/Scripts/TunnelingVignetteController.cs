...
namespace Saritasa.Fader
{
    ...
    {
        [SerializeField]
        [Range(0f, Defaults.fadeOpacityMax)]
        private float fadeOpacity = Defaults.fadeOpacityDefault;
        ...
        {
            /// <summary>
            /// The default maximum value of <see cref="fadeOpacity"/>.
            /// </summary>
            public const float fadeOpacityMax = 1f;
            ...
            /// <summary>
            /// The default value of <see cref="fadeOpacity"/>.
            /// </summary>
            public const float fadeOpacityDefault = 0f;
            ...
            {
                FadeOpacity = fadeOpacityDefault,
        ...
        public void CopyFrom(VignetteParameters parameters)
        {
            ...
            FadeOpacity = parameters.FadeOpacity;
    ...
    public class TunnelingVignetteController : MonoBehaviour
    {
        ...
        {
            public static readonly int FadeOpacity = Shader.PropertyToID("_FadeOpacity");
        ...
        class ProviderRecord
        {
            ...
            public float DynamicFadeOpacity { get; set; } = 0f;
...
#endif
            currentParameters.CopyFrom(VignetteParameters.Defaults.noEffect);
        ...
        // TODO: Break it down into separate methods.
        /// <summary>
        /// See <see cref="MonoBehaviour"/>.
        /// </summary>
        protected virtual void Update()
        {
            ...
            foreach (var record in m_ProviderRecords)
            {
                ...
                var currentOpacity = record.DynamicFadeOpacity;
                ...
                switch (record.easeState)
                {
                    ...
                    case EaseState.EasingIn:
                    {
                        ...
                        var desiredEaseInOpacity = parameters.FadeOpacity;
                        record.easeInLockEnded = false;

                        // Size.
                        if (desiredEaseInTime > 0f && currentSize > desiredEaseInSize)
                        {
                            var updatedSize = currentSize + (desiredEaseInSize - apertureSizeMax) / desiredEaseInTime *
                                Time.unscaledDeltaTime;
                            record.dynamicApertureSize =
                                updatedSize < desiredEaseInSize ? desiredEaseInSize : updatedSize;
                        }
                        else
                        {
                        ...
                        // Opacity.
                        if (desiredEaseInTime > 0f && currentOpacity < desiredEaseInOpacity)
                        {
                            var updatedOpacity = currentOpacity + desiredEaseInOpacity / desiredEaseInTime *
                                Time.unscaledDeltaTime;
                            record.DynamicFadeOpacity =
                                updatedOpacity > desiredEaseInOpacity ? desiredEaseInOpacity : updatedOpacity;
                        }
                        else
                        {
                            record.DynamicFadeOpacity = desiredEaseInOpacity;
                        }
                    ...
                    case EaseState.EasingInHoldBeforeEasingOut:
                    {
                        if (!record.easeInLockEnded)
                        {
                            ...
                            {
                                if (currentOpacity < desiredEaseInOpacity)
                                {
                                    var updatedOpacity = currentOpacity + desiredEaseInOpacity /
                                        desiredEaseInTime * Time.unscaledDeltaTime;
                                    record.DynamicFadeOpacity =
                                        updatedOpacity < desiredEaseInOpacity ? desiredEaseInOpacity : updatedOpacity;
                                }
                    ...
                    case EaseState.EasingOut:
                    {
                        ...
                        {
                            var updatedSize = currentSize + (apertureSizeMax - startSize) /
                                desiredEaseOutTime * Time.unscaledDeltaTime;
                            record.dynamicApertureSize = updatedSize > apertureSizeMax ?
                                                                       apertureSizeMax :
                                                                       updatedSize;
                        }
                        else
                        ...
                        // Opacity.
                        var startOpacity = parameters.FadeOpacity;

                        if (desiredEaseOutTime > 0f && currentOpacity > 0f)
                        {
                            var updatedOpacity = currentOpacity - startOpacity /
                                desiredEaseOutTime * Time.unscaledDeltaTime;
                            record.DynamicFadeOpacity = updatedOpacity < 0f ? 0f : updatedOpacity;
                        }
                        else
                        {
                            record.DynamicFadeOpacity = 0f;
                        ...
                        if (record.dynamicApertureSize >= apertureSizeMax && record.DynamicFadeOpacity <= 0f)
            ...
            foreach (var record in m_ProviderRecords)
            {
                ...
                var fadeOpacity = record.DynamicFadeOpacity;
                if (fadeOpacity > 0f)
                {
                    minRecord = record;
                    minDynamicFadeOpacity = fadeOpacity;
                }
            }

            if (minRecord != null)
                currentParameters.CopyFrom(minRecord.provider.vignetteParameters ?? m_DefaultParameters);

            currentParameters.apertureSize = minDynamicApertureSize;
            currentParameters.FadeOpacity = minDynamicFadeOpacity;
        ...
        void UpdateTunnelingVignette(VignetteParameters parameters)
        {
            ...
            if (TrySetUpMaterial())
            {
                ...
                m_VignettePropertyBlock.SetFloat(ShaderPropertyLookup.FadeOpacity, parameters.FadeOpacity);

...
public abstract class ObjectFollow : MonoBehaviour
{
    ...
    [Tooltip(
        "The game object to change the property values of. If left empty the game object this script is attached to will be changed.")]
    ...
    protected virtual void FollowPosition()
    {
        ...
        if (smoothsPosition)
        {
            float alpha = Mathf.Clamp01(Vector3.Distance(targetPosition, positionToFollow) /
                                        maxAllowedPerFrameDistanceDifference);
    ...
    protected virtual void FollowRotation()
    {
        ...
        if (smoothsRotation)
        {
            float alpha = Mathf.Clamp01(Quaternion.Angle(targetRotation, rotationToFollow) /
                                        maxAllowedPerFrameAngleDifference);
    ...
    protected virtual void FollowScale()
    {
        ...
        {
            float alpha =
                Mathf.Clamp01(Vector3.Distance(targetScale, scaleToFollow) / maxAllowedPerFrameSizeDifference);

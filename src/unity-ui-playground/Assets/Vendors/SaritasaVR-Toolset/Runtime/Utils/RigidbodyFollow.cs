...
/// </summary>
public class RigidbodyFollow : ObjectFollow
{
    /// <summary>
    ...
    [Tooltip(
        "The maximum distance the tracked `Game Object To Change` Rigidbody can be from the `Game Object To Follow` Rigidbody before the position is forcibly set to match the position.")]
    ...
    protected virtual void TrackRotation(Quaternion newRotation)
    {
        ...
        if (angle != 0)
        {
            ...
            Vector3 calculatedAngularVelocity =
                Vector3.MoveTowards(rigidbodyToChange.angularVelocity, angularTarget, maxDistanceDelta);
            if (trackAngularVelocityLimit == float.PositiveInfinity ||
                calculatedAngularVelocity.sqrMagnitude < trackAngularVelocityLimit)
            {
